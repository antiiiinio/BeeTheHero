    ...
    void Update()
    {
        ...
        if (inimigosvivos == 0)
        {
            if (timer >= time)
            {
                if (wave == 3)
                {
                    Instantiate(inimigo, transform.position + new Vector3(2, 3, 0), transform.rotation);
                    Instantiate(inimigo, transform.position + new Vector3(-2, 3, 0), transform.rotation);
                    Instantiate(MiniInimigo, transform.position + new Vector3(0, -3, 0), transform.rotation);
                    Instantiate(MiniInimigo, transform.position + new Vector3(3, -2, 0), transform.rotation);
                    Instantiate(MiniInimigo, transform.position + new Vector3(-3, -2, 0), transform.rotation);
                    timer = 0f;
                    wave++;
                }
